...
<head>
    ...
    <title>Listar Reservas - Sistema Emprestimos</title>
</head>
<body>
    ...
    <div class="container-fluid">
        <div class="row flex-nowrap">
            ...
            <div class="col bg-light vh-100 overflow-auto">
                <div class="navbar">
                    <h1 class="mx-auto">Minhas Reservas</h1>
                </div>
                <table class="table table-striped height-25">
                    ...
                    <tbody>
                        <?php
                            ...
                            $id = $_SESSION['id'];
                            $sql = "SELECT reservas.id, reservas.data_retiro, reservas.dias, reservas.status, materiais.nome AS 'nomeMaterial'
                                    FROM reservas INNER JOIN materiais ON reservas.id_material=materiais.id WHERE reservas.id_usuario = $id";
                            ...
                            if ($result->num_rows > 0) {
                                while ($linha = $result->fetch_assoc()) {
                                    ...
                                    if ($linha["status"] == "Aberto") {
                                        echo "<td>
                                            <a href='cancelarReserva.php?id=" . $linha["id"] . "'onclick='return confirm(" . '"Tem certeza que quer cancelar a reserva?"' . ")'>
                                                Cancelar
                                            </a>
                                        </td>";
                                    }
